...
export const printExecutedOrder = order => console.log(`${new Date(order.transactTime).toLocaleString()} ${order.symbol} ${order.status} ${order.type} ${order.timeInForce} ${order.side} ${order.origQty} at ${order.price}`);
